...
//import com.caccommonmodule.contentprovider.NotePad.Notes;
//
...
public class NotePadProvider extends ContentProvider {
    ...
    static{
        ...
        sNotesProjectionMap.put(Notes.PASSWORD, Notes.PASSWORD);
        sNotesProjectionMap.put(Notes.LOGINDATA, Notes.LOGINDATA);
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
         ...
         * 因为我们的一个ContentValues对象 对应一条数据库的记录
         * */
        if (values.containsKey(Notes.CREATEDDATE) == false) {
            values.put(Notes.CREATEDDATE, now);
        }
        if (values.containsKey(Notes.MODIFIEDDATE) == false) {
            values.put(Notes.MODIFIEDDATE, now);
        }
        if (values.containsKey(Notes.TITLE) == false) {
            //返回一个全局共享的资源对象
            Resources r = Resources.getSystem();
            values.put(Notes.TITLE, r.getString(android.R.string.unknownName));
        }
        if (values.containsKey(Notes.NOTE) == false) {
            values.put(Notes.NOTE, "");
        }

        if (values.containsKey(Notes.ACCOUNT) == false) {
            values.put(Notes.ACCOUNT, "");
        }
        if (values.containsKey(Notes.PASSWORD) == false) {
            values.put(Notes.PASSWORD, "");
        }
        if (values.containsKey(Notes.LOGINDATA) == false) {
            values.put(Notes.LOGINDATA, "");
        }
